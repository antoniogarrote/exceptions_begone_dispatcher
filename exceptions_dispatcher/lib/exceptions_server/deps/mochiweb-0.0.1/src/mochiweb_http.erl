...
loop(Socket, Body) ->
    error_logger:info_msg("IN MY LOOP",[]),
    inet:setopts(Socket, [{packet, http}]),
    request(Socket, Body).

request(Socket, Body) ->
    error_logger:info_msg("ABOUT TO RECEIVE WITH TIMEOUT ~p",[?IDLE_TIMEOUT]),
    case gen_tcp:recv(Socket, 0, ?IDLE_TIMEOUT) of
        {ok, {http_request, Method, Path, Version}} ->
            error_logger:info_msg("HTTP REQUEST",[]),
            headers(Socket, {Method, Path, Version}, [], Body, 0);
        {error, {http_error, "\r\n"}} ->
            error_logger:info_msg("HTTP ERROR \r\n",[]),
            request(Socket, Body);
        {error, {http_error, "\n"}} ->
            error_logger:info_msg("HTTP ERROR \n",[]),
            request(Socket, Body);
        Other ->
            error_logger:info_msg("CLOSING 1 ~p",[Other]),
...
headers(Socket, Request, Headers, _Body, ?MAX_HEADERS) ->
    ...
    Req:respond({400, [], []}),
    error_logger:info_msg("CLOSING 2"),
    gen_tcp:close(Socket),
    exit(normal);
headers(Socket, Request, Headers, Body, HeaderCount) ->
    case gen_tcp:recv(Socket, 0, ?IDLE_TIMEOUT) of
        {ok, http_eoh} ->
            ...
            Req = mochiweb:new_request({Socket, Request,
                                        lists:reverse(Headers)}),
            error_logger:info_msg("MOCHIWEB INVOKING MAIN LOOP BEFORE",[]),
            Body(Req),
            error_logger:info_msg("MOCHIWEB INVOKING MAIN LOOP AFTER",[]),
            case Req:should_close() of
                true ->
                    error_logger:info_msg("CLOSING 3"),
                    gen_tcp:close(Socket),
                    exit(normal);
                false ->
                    case Req:get_header_value("upgrade") of
                        "WebSocket"  -> error_logger:info_msg("MOCHIWEB HTTP WEB_SOCKET!",[]),
                                        gen_tcp:recv(Socket, 0),
                                        dont_loop ;
                        _RegularHTTP -> Req:cleanup(),
        ...
        {ok, {http_header, _, Name, _, Value}} ->
            headers(Socket, Request, [{Name, Value} | Headers], Body,
                    1 + HeaderCount);
        _Other ->
            error_logger:info_msg("CLOSING 4"),
            gen_tcp:close(Socket),
            exit(normal)
    end.
